...
--------
    Run edk from parent directory in verbosity mode with mpi on 4 processes:
        python run_edk_checks.py -e ../edk -v -m 4

    Silently run edk (given with an absolute path) with openmp on 4 threads:
        python run_edk_checks.py -e /abspath/edk_openmp -t 4

    Silently run edk from parent directory:
        python run_edk_checks.py

    Run with multiple edk exes:
        python run_edk_checks.py -e ../edk1 ../edk2
"""
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=EXE,
        dest="exe",
        help="Paths to edk exe[s]. (default: {})".format(EXE),
    )
    parser.add_argument(
        "-v",
        "--verbose",
        ...
        default=False,
        dest="verbose",
        help="Show the edk output. (default: no)",
    )
    parser.add_argument(
        "-l",
        "--log_path",
        ...
        default=None,
        dest="log_path",
        help="Directory for edk-logs. (default: the resp. case dir)",
    )
    parser.add_argument(
        "-m",
        "--mpi",
...
def run_model(
    ...
    """
    Run edk in a given directory.

    Parameters
    ----------
    exe : str
        path to the edk executable.
    cwd : str or None
        path to the working directory to run edk in. Default: os.get_cwd()
    mpi_nop : int
    ...
    save_log : bool, optional
        Whether to save the edk output to a file.
        Default: True
    ...
    timeout : int or None, optional
        Time to wait for edk to finish in seconds. Default: 1800

    Returns
    -------
    success : bool
        State if edk terminated 'normally'.
    """
    # use absolute path since we change the cwd in the edk call
    ...
    if mpi and int(openmp_threads) > 0:
        raise ValueError("edk run: either use mpi or openmp, not both.")
    ...
    # check the last 4 lines of the log-file for "IS FINISHED!"
    with open(log) as log_f:
        lines = log_f.readlines()
    edk_ok = any(["IS FINISHED!" in line for line in lines[-8:]])
    success = exit_ok and edk_ok
...
if __name__ == "__main__":
    ...
    if int(mpi_nop) > 0:
        skip += SKIP_CASES_MPI
    # iterate of all edk exe-s given
    for exe in exe_list:
    ...
    if not final_result:
        raise AssertionError("edk checks were not successful.")
