...
path_to_stationdata = '/data/stohyd/data/processed/Germany/DWD/pre/pre_1900-2015/'
stat_id             = '00001'
netcdfpathbase      = '/work/zink/edk_crossloc/pre/'
...
parser.add_option('-l', '--lut', action='store',
                  default=stat_id, dest='stat_id', metavar='stat_id',
...
statsfile = netcdfpathbase + '/' + stat_id + '_interpolation_vs_station.crosscheck.stat.txt'


if (not ommit_plotting):
    #pdffile   = ''
    pdffile   = netcdfpathbase + '/' + stat_id + '_interpolation_vs_station.crosscheck.pdf'
...
# netcdf name
netcdffile = netcdfpathbase + 'exclude_' + str(stat_id).zfill(5) + '/pre.nc'
...
fname_station = path_to_stationdata + stat_id + '.dat'
...
# error estimation
bias     = np.mean(netcdf_tseries_final) -  np.mean(stat_tseries_final)
rel_bias = (np.mean(netcdf_tseries_final) -  np.mean(stat_tseries_final)) / np.mean(stat_tseries_final) * 100.0
rmse     = np.sqrt (np.mean( (netcdf_tseries_final-stat_tseries_final)**2 ) )
rel_rmse = np.sqrt (np.mean( (netcdf_tseries_final-stat_tseries_final)**2 ) ) * 100.0 /np.mean(stat_tseries_final)
pear     = scim.pearsonr(netcdf_tseries_final,stat_tseries_final)[0]

# print (ufz.astr(bias,2) + ' ' + ufz.astr(rel_bias,2) + ' ' + ufz.astr(rmse,2) + ' ' +
#    ufz.astr(rel_rmse,2) + ' ' + ufz.astr(pear,2))
...
if (not ommit_plotting):
    ...
    ax.set_title(str(stat_id))
    ax.set_ylabel('Precipitation [mm d-1]')

    ylim = np.mean(stat_tseries_final[stat_tseries_final>0])*10
    ax.text ( dates_cut[500], ylim*0.95, "BIAS  " + ufz.astr(bias,2),     ha='left')
    ax.text ( dates_cut[500], ylim*0.92, "rBIAS " + ufz.astr(rel_bias,2), ha='left')
    ax.text ( dates_cut[500], ylim*0.89, "RMSE  " + ufz.astr(rmse,2),     ha='left')
    ax.text ( dates_cut[500], ylim*0.86, "rRMSE " + ufz.astr(rel_rmse,2), ha='left')
    ax.text ( dates_cut[500], ylim*0.83, "PEARS " + ufz.astr(pear,2),     ha='left')
...
f = open(statsfile, 'w')
f.write('ID,Bias[mm_d-1],rBias[%],RMSE[mm_d-1],rRMSE[%],PEARS[mm_d-1]\n')
f.write(str(stat_id) + ',' +  ufz.astr(bias,2) + ',' + ufz.astr(rel_bias,2) + ',' + ufz.astr(rmse,2) + ',' +
        ufz.astr(rel_rmse,2) + ',' + ufz.astr(pear,2) + '\n')
f.close()    
